...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let ind = self
            .chunks
            .iter()
            .position(|p| *p.chunk_type() == chunk)
            .unwrap();
        let remove = self.chunks.remove(ind);
        Ok(remove)
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        // let mut reader = BufReader::new(chunk_type.as_bytes());
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(Box::new(ChunkTypeError::InvalidHeader));
        }
